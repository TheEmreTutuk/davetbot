...
fs.readdir('./commands/', async (err, files) => {
    if(err){
        return console.log(chalk.red('Yüklenecek komutlar için komutlar klasörünü kontrol ederken bir hata oluştu: ' + err));
    }
...
client.on('ready', async () => {
  ...
  client.guilds.cache.forEach(guild => {
    ...
    if(botstatus.activity_type.toUpperCase() == 'PLAYİNG') {
...
client.on('guildMemberAdd', async member => {
  if(process.env.welcomeChannel != "false") {
    ...
    if(member.user.bot) return welcomeChannel.send(`<@${member.id}> OAuth akışını kullanarak sunucuya katıldı.`)
  }
  ...
  try {
    ...
    if(currentInvites) {
      ...
      if(welcomeChannel) {
        welcomeChannel.send(`<@${member.id}> Sunucuya Katıldı. Davet Eden Kişi **${usedInvite.inviter}** (${newamount} Daveti Var).`).catch(err => console.log(err));
      }
      ...
      db.set(`${member.id}`, `${usedInvite.inviter.id}`)
      if(welcomeChannel) {
        welcomeChannel.send(`<@${member.id}> Sunucuya Katıldı. Davet Eden  Kişi **${usedInvite.inviter}** (1 Daveti Var).`).catch(err => console.log(err));
      }
    }
  }
  catch(err) {
...
client.on('guildMemberRemove', async member => {
  if(process.env.welcomeChannel != "false") {
    const welcomeChannel = client.channels.fetch(process.env.welcomeChannel)
    if(member.user.bot) return welcomeChannel.send(`<@${member.id}> sunucudan ayrıldılar, OAuth aracılığıyla katıldılar.`)
  }
  const db = require('./db.js')
  ...
  try {
    ...
    const newamount = currentInvites-0 - 1-0
    if(welcomeChannel) {
      welcomeChannel.send(`${member.user.tag} Sunucudan Ayrıldı. Davet Etmiş Olan **${userinviter.user.tag}** (Simdi ${newamount} Daveti Var).`).catch(err => console.log(err));
    }
