exports.run = async (client, message, args) => {
  ...
  if (!message.member.roles.cache.some(role =>["791041152605880361"].includes(role.id))) {
    ...
    if(mention) {
      if(args[1]) {
        ...
        if(invites) {
          ...
          if(newamount.toString() == 'NaN') {
            ...
            message.channel.send({embed: {
              description: `Silindi ${args[1]} Davet Sayısı <@${mention.id}>! Simdi ${newamount} Daveti Var!`,
              footer: {
                text: client.user.username,
                icon_url: client.user.displayAvatarURL()
              },
          ...
          if(newamount.toString() == 'NaN') {
            ...
            db.set(mention.id, newamount)
            message.channel.send({embed: {
              description: `Silidni ${args[1]} Davet Sayısı <@${mention.id}>! Simdi ${newamount} Daveti Var!`,
              footer: {
                text: client.user.username,
                icon_url: client.user.displayAvatarURL()
              },
        ...
        if(invites) {
          ...
          if(newamount.toString() == 'NaN') {
            ...
            const question = message.channel.send({embed: {
              description: `Bu Davet Sayısı Silmek mi İstedin <@${found.id}> (${found.user.tag})?`,
              footer: {
            ...
            message.channel.awaitMessages(filter, { max: 1, time: 60000 }).then(async collected => {
              if (collected.size === 0) {
                ...
                if(answer === 'yes') {
                  ...
                  message.channel.send({embed: {
                    description: `Silindi ${args[1]} Davet Sayısı <@${found.id}>! Simdi ${newamount} Daveti Var!`,
                    footer: {
                      text: client.user.username,
                      icon_url: client.user.displayAvatarURL()
                    },
      ...
      message.channel.send({embed: {
        description: `Kimin Daveti Sileceksin?`,
        footer: {
          text: client.user.username,
          icon_url: client.user.displayAvatarURL()
        },
